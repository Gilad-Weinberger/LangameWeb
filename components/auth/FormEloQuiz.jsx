...
const FormEloQuiz = ({ prevStep, setEloScore, submitForm, values }) => {
  ...
  const handleAnswer = (value) => {
    ...
    if (currentQuestion < quizQuestions.length - 1) {
      ...
      // Automatically submit the form after the last question
      submitForm();
      setCompleted(true);
    }
  };
        ...
        <div className="text-center">
          <div className="mb-8">
            ...
            <p className="text-text-secondary">
              המשיכו להגדיל את רמת היכולות שלכם ולהפגין את עצמכם במשחקים
            </p>
          </div>
          <div className="space-y-3">
            <Link
              href="/dashboard"
              className="w-full bg-main hover:bg-main-hover text-white py-3 px-4 rounded-lg font-medium transition-colors"
            >
              המשך
            </Link>
          </div>
